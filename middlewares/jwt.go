...
func (j *JWT) Middleware(next echo.HandlerFunc) echo.HandlerFunc {
	...
	return func(c echo.Context) error {
		...
		decryptedToken, err := DecryptToken(accessToken, []byte(cfg.SECRETKEY_AUTH))
		if err != nil {
			c.String(400, "Unauthorized: inappropriate access token format")
			return nil
		}
		decryptedRToken, err := DecryptToken(refreshToken, []byte(cfg.SECRETKEY_REFRESH))
		if err != nil {
		...
		if int64(tokenExpirationDate) <= time.Now().Unix() || int64(rTokenExpirationDate) <= time.Now().Unix() {
			...
			uuid := c.Request().Header.Get("login")
			responseRefresh, err := j.client.RefreshToken(context.Background(), &protocol.RefreshTokenRequest{Uuid: uuid, Token: accessToken})
			if err != nil {
